...
public class TableV1ApiImpl implements TableV1Api, ApiUtils {
    ...
    public Response createTableInProvider(String providerName, CreateTableInput createTableInput) {
        return wrapExceptions(() ->
                        Response.status(Response.Status.CREATED).entity(Mappers.internalTable2api(tableService.createInternalTable(
                                providerName,
                                getRequestPrincipal(),
    ...
    @Override
    public Response patchTableInProvider(String providerName, String tableName, PatchTableInput patchTableInput) {
        return Response.status(501).build();
    }
