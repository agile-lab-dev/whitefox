...
public class DeltaSharedTable implements InternalSharedTable {
    ...
    public static DeltaSharedTable of(
        ...
        if (sharedTable.internalTable().properties() instanceof InternalTable.DeltaTableProperties) {
            ...
            var tableClient = DefaultTableClient.create(hadoopConfigBuilder.buildConfig(sharedTable.internalTable().provider().storage()));
            Table dt = null;
            try {
    ...
    public ReadTableResultToBeSigned queryTable(ReadTableRequest readTableRequest) throws TableNotFoundException {
